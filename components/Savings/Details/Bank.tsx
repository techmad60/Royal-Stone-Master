...
import { MakeSavingsBankResponse } from "@/types/Type";
import { useEffect } from "react";
import { TbTargetArrow } from "react-icons/tb";

interface BankDetailsProps {
  onClose: () => void;
  purchaseResponse: MakeSavingsBankResponse["data"] | null;
  name: string;
}
        ...
        <div className="flex items-center gap-4 my-1 p-4">
          ...
          <div className="flex flex-col gap-1">
            ...
            <p className="text-color-form text-sm">
              ...
              <span
                className={`${
                  purchaseResponse?.savings?.status?.toLowerCase() === "pending"
                    ? "text-yellow-500"
                    : purchaseResponse?.savings?.status?.toLowerCase() === "successful"
                    ? "text-color-one"
                    : "text-red-500"
                }`}
              >
                {purchaseResponse?.savings?.status?.toUpperCase()}
              </span>
            </p>
          </div>
        </div>
        ...
        <section className="self-center grid grid-cols-2 bg-light-grey rounded-[10px] shadow-sm mx-6 mt-4 p-[15px] text-sm w-[345px] h-[155px] lg:p-5 lg:w-[572px] lg:h-[155px]">
          <div className="flex flex-col space-y-8">
            <div className="flex flex-col">
              ...
              <p className="text-color-six">${purchaseResponse?.savings?.amount}</p>
            </div>
            <div className="flex flex-col">
              <p className="text-color-form">Transaction Date</p>
              <p className="text-color-six">
                {purchaseResponse?.savings?.createdAt
                  ? new Date(purchaseResponse?.savings?.createdAt).toLocaleDateString(
                      "en-GB",
                      {
                        day: "numeric",
                        month: "numeric",
          ...
          <div className="flex flex-col space-y-8">
            ...
            <div className="flex flex-col">
              ...
              <p className="text-color-six">
                {purchaseResponse?.savings?.createdAt
                  ? new Date(purchaseResponse.savings?.createdAt)
