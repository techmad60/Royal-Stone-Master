...
interface SavingsProcessedProps {
  onClose: () => void;
  onConfirm: () => void;
}

export default function SavingsProcessed({
  onClose,
  onConfirm,
}: SavingsProcessedProps) {
    ...
    <div className="fixed inset-0 flex bg-[#D9D9D9A6] bg-opacity-50 z-50 items-end justify-center lg:items-start lg:justify-end lg:bg-transparent">
      <div className="bg-color-one flex flex-col space-y-4 w-full max-w-md p-4 lg:mt-20 lg:rounded-2xl lg:mr-8">
        ...
        <p className="text-sm font-medium text-[rgba(255,255,255,1)]">
          Your Annual Rent Savings Target has been created successfully
        </p>
        <div
          onClick={onConfirm}
          className="text-center flex justify-center items-center bg-[rgba(255,255,255,0.1)] rounded-[8px] border border-[rgba(255,255,255,0.1)]"
        >
          <Button
